...
Build a typed node graph, run a real image pipeline, compare before/after, and export. No backend, no account, and no data leaving your computer.
...
**AI Edit** takes an image plus a plain-language instruction, such as *"remove the car"* or *"make it snow,"* and applies it to the whole frame, with no mask to paint. Paint Mask remains available as a utility node; mask-scoped editing is planned for a later release.
